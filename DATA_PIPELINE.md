...
3. CSVの列・件数・必須値・供給区分・YJコード・日付・鮮度を検査
   続けて `scripts/check_web_client_csv.py` が、公開中の `index.html` のCSV読込検証そのものを
   Nodeで実行し、Webが読めない形式なら公開前に停止する（2026年9月16〜19日の検索停止の再発防止。
   データ側で新しい値を許可するときは、Web側の対応を先に公開する）
...
鮮度は2種類を分けて判定する。`version.json` は毎日の実行日付なので暦日4日以内、
CSV最新行の更新日は厚労省の開庁日にしか進まないため、土日祝・年末年始（12/29〜1/3）を
除く3営業日以内（`--csv-max-age-business-days 3`）とする。祝日は
`scripts/jp_business_days.py` が現行の祝日法の規則で計算し、臨時の祝日や法改正は含まない。
対応年の範囲外は通過させずに異常として報告する。

厚労省の供給状況Excelは週1回程度の公開で、日次更新は同じファイルを取り込み直す日が多い。
新しいファイルを取得しても後段の検査で止まると鮮度監視（営業日）では気付けないため
（2026-09-26の障害）、日次更新は取り込んだファイル名と日付を `mhlw_source.json` に記録し、
コア公開と同じcommitに含める。公開後・定期の監視は厚労省ページの最新Excelの日付と比べ、
前日以前に公開されたファイルが未反映なら異常とする。公開当日は待機扱い、厚労省ページに
接続できない・ファイル名の形式が変わった場合は警告に留める。
...
- 週次まとめ：前週（月〜日、日本時間）に変更記録があれば、月〜水曜の実行で1回だけ
  投稿する（リンクは `updates/index.html`）。記録がない週は投稿しない。
- 月次レポート告知：`reports/YYYY-MM.json` の作成から7日以内に1回だけ投稿する
  （リンクは `reports/YYYY-MM.html`）。
- 二重投稿防止の記録は種類ごとのキー（日別=日付、`weekly:週初日`、`monthly:YYYY-MM`）。
